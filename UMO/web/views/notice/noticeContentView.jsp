...
%>
   ...
   <style>
    section#notice-container{margin:0 auto; text-align:center; margin-top:150px; }
   ...
   <h2>공지사항</h2>
        ...
        <tr>
            ...
            <%if(nb.getRenamed_filename()!=null) {%>
						...
						width="16px"/></a>
					<%}else if(nb.getOriginal_filename()!=null){ %>
					<a href="<%=request.getContextPath()%>/notice/fileDown?fileName=<%=nb.getOriginal_filename() %>">
						<img src="<%=request.getContextPath()%>/images/file.png"
						width="16px"/></a>
					<%} %>
            </td>
        ...
        <tr>
            <th colspan="2">
                ...
                <input type="button" class="btn btn-list" value="목록" onclick="location.href='<%=request.getContextPath() %>/noticeBoard?noticeNo=<%=nb.getNo() %>'"/>
    ...
    <div id="comment-container">
			<div class="comment-editor">
				<form action="<%=request.getContextPath()%>/notice/insertComment"
		...
		%>
			<tr class='level1'>
				...
				<td>
					...
					<button class="btn-delete" value="<%=bc.getBoardCommentNo()%>" onclick="location.href='<%=request.getContextPath()%>/notice/deleteComment?noticeNo=<%=bc.getBoardCommentNo()%>'">삭제</button>
