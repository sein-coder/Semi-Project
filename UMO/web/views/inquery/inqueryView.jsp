...
<% 
	...
	Inquery prenb=(Inquery)request.getAttribute("preb");
	Inquery nextnb=(Inquery)request.getAttribute("nextb");
	...
	<div id="container_wr">
		...
		<article id="bo_v" style="width: 100%; height: auto !important;">
			...
			<section id="bo_v_info">
				<h2>페이지 정보</h2>
				<strong><span
					class="sv_member"><%=inquery.getBoard_Writer()%></span></strong><strong><a
			...
			<div id="bo_v_top">
				...
				<ul class="bo_v_com">
					...
					<a href="<%=request.getContextPath()%>/scrap/scrapViewServlet"
						class="btn_b01 btn"><i class="fa fa-list" aria-hidden="true"></i>
							목록</a>
					<% } else { %>
					<a href="<%=request.getContextPath()%>/inquery/inqueryBoard" class="btn_b01 btn"><i class="fa fa-list" aria-hidden="true"></i>
							목록</a>
					<% } %>		
					</li>
				...
				<ul class="bo_v_nb">
					<li class="btn_prv"><span class="nb_tit"><i
						...
						<% if(prenb!=null&&prenb.getBoard_Title()!=null) { %>
						<a href="<%=request.getContextPath()%>/inquery/inqueryView?Board_No=<%=prenb.getBoard_No()%>"><%= prenb.getBoard_Title() %></a> 
						<% } else { %>
						<a href="">이전 글이 없습니다.</a> 
						<% } %>
						<span class="nb_date"><%= prenb!=null?prenb.getBoard_Date():"" %></span></li>
					<li class="btn_next"><span class="nb_tit"><i
						class="fa fa-caret-down" aria-hidden="true"></i> 다음글</span>
						<% if(nextnb!=null&&nextnb.getBoard_Title()!=null) { %>
						<a href="<%=request.getContextPath()%>/inquery/inqueryView?Board_No=<%=nextnb.getBoard_No()%>"><%= nextnb.getBoard_Title() %></a> 
						<% } else { %>
						<a href="">다음 글이 없습니다.</a> 
						<% } %>
						<span class="nb_date"><%= nextnb!=null?nextnb.getBoard_Date():"" %></span></li>
				</ul>
			</div>
			<!-- } 게시물 상단 버튼 끝 -->
			...
			<section id="bo_vc">
				...
				<% if(!list.isEmpty()) {
					...
					<header style="z-index: 2">
						<h2><%=bc.getComment_writer() %>님의 답변</h2>
			...
			<aside id="bo_vc_w" class="bo_vc_w">
				...
				<form name="fviewcomment" id="fviewcomment"
					action="<%= request.getContextPath() %>/inqueryComment/insertComment"
