...
<style>
	...
	table#big-tbl{ margin-left: auto; margin-right: auto; border-spacing: 50px 20px; zoom:0.7; }
	table#sml-tbl{ border: 1px solid black;}
					...
					<% if(count==0 || count==4)  {%><tr>
						...
						<td>
							<table id="sml-tbl">
								<tr>
									<td colspan="2"><a
										href='<%=request.getContextPath()%>/food/foodView?board_no=<%=f.getBoard_No()%>&cPage=<%=cPage%>&tag=<%=tag.equals("")?"":tag%>'>
											<img class="foodimg" alt="img"
											src="<%=request.getContextPath() %>/upload/food/thumnail/<%= f.getBoard_Thumbnail() %>">
									</a></td>
								</tr>
								<tr>
							...
							<div class="content">
								<ul id="tag-list">
									...
									<li class="tag-item"><a
										href='<%=request.getContextPath()%>/food/foodTagSearch?tag=<%=str%>'>#<%= str %></a></li>
									<% } 
						        	} else { %>
									<% } %>
								</ul>
