	...
	<div id="memo_list" class="new_win">
		...
		<div class="new_win_con">
			<ul class="win_ul">
				...
				<li class="selected"><a href="<%=request.getContextPath()%>/sendMemoListServlet?userId=<%= userId %>">보낸쪽지</a></li>
				<li><a href="<%=request.getContextPath()%>/memoWriteServlet?userId=<%= userId %>">쪽지쓰기</a></li>
			</ul>
			<div class="win_total">
				<span> 전체 보낸쪽지 <%= list.size() %>통<br>
				</span>
			</div>
			<div class="list_01">
				...
				<ul>
					...
					<li><span class="memo_name"><a
							href="<%=request.getContextPath()%>/memoContentView?memoNo=<%= m.getNotes_no() %>&userId=<%=userId%>&type=send"><%= m.getSend_id()!=null?m.getSend_id():"익명" %></a>
